...
//to fetch all widasie maryam categories to show on 'ውዳሴ ማርያም በግእዝ' list
 ...
 //fetch daily pray without divide in sub item like "ፈቀደ እግዚእ፣ ሠረቀ በሥጋ"
  Future<List<Map<String, dynamic>>?> getPreyInGeez(id) async {
    if (_database == null) {
      await initDB();
    }

    String query = 'SELECT nibab_id, content FROM nibab WHERE category_id=$id';
    List<Map<String, dynamic>>? result = await _database?.rawQuery(query);
    return result;
  }
